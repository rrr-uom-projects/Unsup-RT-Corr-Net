...
from utils import getDirs, windowLevelNormalize
...
def f(fname, corr_or_ms, lung_apex_cut, optimise=True):
    ...
    #im_patches = sample_ct_on_vertices(vertices, ct, spacing, patch_size=(5,13,13))
...
def __main__():
    ...
    for structure_name in tqdm(structs):
        ...
        processes = [Process(target=f, args=(fname, reg_style, lung_apex_cut)) for fname, reg_style, lung_apex_cut in zip(fnames, RIGID_REG_STYLE, LUNG_APEX_CUTS)]
        ...
        for fname, reg_style, lung_apex_cut in zip(fnames, RIGID_REG_STYLE, LUNG_APEX_CUTS):
            if not os.path.exists(join(output_dir, "meshes/", f"{outf_name}_{fname}.ply")):
                ...
                processes.append(Process(target=f, args=(fname, reg_style, lung_apex_cut, False)))
        ...
        for fname in fnames:
            if not os.path.exists(join(output_dir, "meshes/", f"{outf_name}_{fname}.ply")):
                print(f"Mesh still not created for {fname} - likely empty mask or another errors...", flush=True)

        # report that all tasks are completed
        print(f'Done {structure_name}', flush=True)
