...
class Shape:
    ...
    def detach(self):
        self.verts = self.verts.detach()
        self.triangles = self.triangles.detach()
        if self.normal is not None:
            self.normal = self.normal.detach()
        if self.neigh is not None:
            self.neigh = self.neigh.detach()
        if self.D is not None:
            self.D = self.D.detach()
        if self.verts_full is not None:
            self.verts_full = self.verts_full.detach()
        if self.samples is not None and torch.is_tensor(self.samples):
            self.samples = self.samples.detach()
        if self.sub is not None:
            for i_s in range(len(self.sub)):
                for i_p in range(len(self.sub[i_s])):
                    self.sub[i_s][i_p] = self.sub[i_s][i_p].detach()
