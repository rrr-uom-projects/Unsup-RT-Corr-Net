...
from neuromorph_w_imaging.train import NetParam
import sys
import os
import pickle
...
def run_test(fold_num):
    # find the model to load
    models_dir = ""
    model_name = list(filter(lambda x: f"w_imaging_fold{fold_num}" in x, getDirs(models_dir)))[0]
    ...
    interpol.interp_module.param.num_timesteps = 3
    
    res_name = f""
    save_seq_collection_hard_correspondences(interpol, save_path(folder_str=model_name), res_name)
