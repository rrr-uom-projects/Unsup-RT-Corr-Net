...
class InterpolationModGeoEC(InterpolationModBase):
    def __init__(self, interp_energy, param):
        ...
        self.im_module = more_context_net(hidden_dim=param.hidden_dim, out_dim=param.hidden_dim)
        self.Pi = None
        self.Pi_inv = None
    ...
    def match(self, shape_x, shape_y):
        ...
        im_feat_x = self.im_module(shape_x.im_patches)
        im_feat_y = self.im_module(shape_y.im_patches)

        im_feat_x = im_feat_x / im_feat_x.norm(dim=1, keepdim=True)
        im_feat_y = im_feat_y / im_feat_y.norm(dim=1, keepdim=True)

        feat_x = torch.cat((feat_x, im_feat_x), dim=1)
        feat_y = torch.cat((feat_y, im_feat_y), dim=1)
