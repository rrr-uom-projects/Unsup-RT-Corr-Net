...
class InterpolationModGeoEC(InterpolationModBase):
    ...
    def compute_loss(self, shape_x, shape_y, point_pred_arr, n_normalize=201.0):
        ...
        y_p, y_d, y_h, y_w = shape_y.im_patches.squeeze(0).shape
        x_p, x_d, x_h, x_w = shape_x.im_patches.squeeze(0).shape
        E_imaging = (
                self.param.lambd_imaging
                * (
                    (
                        torch.mm(self.Pi, shape_y.im_patches.squeeze(0).view(y_p, y_d*y_h*y_w))
                        - shape_x.im_patches.squeeze(0).view(x_p, x_d*x_h*x_w)
                    )
                    ** 2
                ).mean()
            )
        ...
        E = E_x_0 + E_align + E_geo + E_imaging
        ...
        return E, [E - E_align - E_geo - E_imaging, E_align, E_geo, E_imaging]
...
class InterpolNet:
    ...
    def train(self):
        ...
        while self.i_epoch < self.interp_module.param.num_it:
            ...
            for i, data in enumerate(tqdm(self.train_loader)):
                ...
                loss_imaging.update(loss_comp[3].item())
                loss.backward()
            ...
            print(f"epoch {self.i_epoch:04d}, loss = {loss_total.avg:.5f} (arap: {loss_arap.avg:.5f}, reg: {loss_reg.avg:.5f}, geo: {loss_geo.avg:.5f}), imaging: {loss_imaging.avg:.5f}, reserved memory={torch.cuda.memory_reserved(0) // (1024 ** 2)}MB")
            ...
            self._log_stats('train', "imaging", loss_imaging.avg)
    ...
    def validate(self, data_loader=None, log=True, plot_example=False):
        ...
        if log:
            print(f"Validation loss = {loss_total.avg:.5f} (arap: {loss_arap.avg:.5f}, reg: {loss_reg.avg:.5f}, geo: {loss_geo.avg:.5f}, imaging: {loss_imaging.avg:.5f})")
            ...
            self._log_stats('val', "imaging", loss_imaging.avg)
